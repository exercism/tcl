...
test parallel-letter-frequency-1 "no texts" -body {
    calculate {}
} -returnCodes ok -match dictionary -result {}

skip parallel-letter-frequency-2
test parallel-letter-frequency-2 "one text with one letter" -body {
    calculate {
        "a"
    }
} -returnCodes ok -match dictionary -result {a 1}

skip parallel-letter-frequency-3
test parallel-letter-frequency-3 "one text with multiple letters" -body {
    calculate {
        "bbcccd"
    }
} -returnCodes ok -match dictionary -result {b 2 c 3 d 1}

skip parallel-letter-frequency-4
test parallel-letter-frequency-4 "two texts with one letter" -body {
    calculate {
        "e"
        "f"
    }
} -returnCodes ok -match dictionary -result {e 1 f 1}

skip parallel-letter-frequency-5
test parallel-letter-frequency-5 "two texts with multiple letters" -body {
    calculate {
        "ggh"
        "hhi"
    }
} -returnCodes ok -match dictionary -result {g 2 h 3 i 1}

skip parallel-letter-frequency-6
test parallel-letter-frequency-6 "ignore letter casing" -body {
    calculate {
        "m"
        "M"
    }
} -returnCodes ok -match dictionary -result {m 2}

skip parallel-letter-frequency-7
test parallel-letter-frequency-7 "ignore whitespace" -body {
    calculate {
        "   "
        "\t"
        "\r\n"
...
skip parallel-letter-frequency-8
test parallel-letter-frequency-8 "ignore punctuation" -body {
    calculate {
        "!"
        "?"
        ";"
...
skip parallel-letter-frequency-9
test parallel-letter-frequency-9 "ignore numbers" -body {
    calculate {
        "1"
        "2"
        "3"
...
skip parallel-letter-frequency-10
test parallel-letter-frequency-10 "Unicode letters" -body {
    calculate {
        "本"
        "φ"
        "ほ"
...
} -returnCodes ok -match dictionary -result {本 1 φ 1 ほ 1 ø 1}

skip parallel-letter-frequency-11
test parallel-letter-frequency-11 "combination of lower- and uppercase letters, punctuation and white space" -body {
    calculate {
...
} -returnCodes ok -match dictionary -result {a 32 b 4 c 6 d 14 e 37 f 7 g 8 h 29 i 19 k 6 l 12 m 7 n 19 o 22 p 7 r 17 s 16 t 30 u 9 v 2 w 9 y 4}

skip parallel-letter-frequency-12
test parallel-letter-frequency-12 "large texts" -body {
    calculate {
...
} -returnCodes ok -match dictionary -result {a 845 b 155 c 278 d 359 e 1143 f 222 g 187 h 507 i 791 j 12 k 67 l 423 m 288 n 833 o 791 p 197 q 8 r 432 s 700 t 1043 u 325 v 111 w 223 x 7 y 251}

skip parallel-letter-frequency-13
test parallel-letter-frequency-13 "many small texts" -body {
    calculate {
        "abbccc"
        "abbccc"
        "abbccc"
...
} -returnCodes ok -match dictionary -result {a 50 b 100 c 150}
